...
# print(Xtrain.shape,Xtest.shape,ytrain,ytest)
n_components_train = 300
n_components_test = 100
pca_train = PCA(n_components=n_components_train, svd_solver='randomized',
          whiten=True).fit(Xtrain)
eigenfaces_train = pca_train.components_.reshape((300, 64*64))

pca_test = PCA(n_components=n_components_test, svd_solver='randomized',
          whiten=True).fit(Xtest)
eigenfaces_test= pca_test.components_.reshape((100, 64*64))
